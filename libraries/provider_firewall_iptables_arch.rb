...
#
class Chef
  class Provider::FirewallIptablesArch < Chef::Provider::LWRPBase
    include FirewallCookbook::Helpers
    include FirewallCookbook::Helpers::Iptables

    provides :firewall, os: 'linux', platform_family: %w(arch) do |node|
      node['platform_family'] == 'arch' || node['firewall']['arch_iptables']
    end
    ...
    action :install do
      ...
      converge_by('install iptables and enable/start services') do
        ...
        directory 'create /etc/iptables.d' do
          path '/etc/iptables.d'
          owner'root'
          group 'root'
          mode '0755'
        ...
        iptables_commands(new_resource).each do |svc|
          ...
          file "create empty /etc/iptables.d/#{svc}" do
            path "/etc/iptables.d/#{svc}"
            content '# created by chef to allow service to start'
            not_if { ::File.exist?("/etc/iptables.d/#{svc}") }
          end
    ...
    action :restart do
      ...
      iptables_commands(new_resource).each do |iptables_type|
        iptables_filename = "/etc/iptables.d/#{iptables_type}"
